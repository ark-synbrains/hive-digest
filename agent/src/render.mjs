...
export function buildIssue({ date, byCategory, sectionOrder }) {
  const order = (sectionOrder?.length ? sectionOrder : DEFAULT_ORDER).filter(
    (k) => (byCategory[k] || []).length
  );
  const subject = `Hive Digest — ${teaser(byCategory, order)} (${date})`;
  ...
  let html = `<!DOCTYPE html><html lang="en" data-theme="dark" style="color-scheme:dark;"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><meta name="color-scheme" content="dark only"/><meta name="supported-color-schemes" content="dark"/><title>Hive Digest — ${escapeHtml(date)}</title><style>:root{color-scheme:dark only;}@media (prefers-color-scheme:light){body{background:${HIVE.bg}!important;color:${HIVE.ink}!important;}}</style></head>`;
  ...
  html += `<p style="margin:10px 0 0;font-size:13px;color:${HIVE.purple};line-height:1.6;">${escapeHtml(date)}</p>`;
